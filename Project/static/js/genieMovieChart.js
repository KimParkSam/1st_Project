window.addEventListener('DOMContentLoaded', event => {
        ...
        if(btnId*viewCount > data.length) {                
            ...
            for(let i=btnId*viewCount-viewCount; i < data.length; i++) {
                temp += `<tr>
                    ...
                    <td class="table-dark"><span>${data[i].rankVariance}</span></td>
    ...
    window.dateHourChange = () => {
        ...
        const url = '/genieMovieChart/'+viewTime;
        ...
        axios({
            ...
            // console.log(response.data.result.genieMoviedata.data);
            ...
            let fileHour = response.data.result.genieMoviedata.fileHour;
            if (fileHour == undefined) {
                fileHour = response.data.result.genieMoviedata.filelist[response.data.result.genieMoviedata.filelist.length-1].slice(27, -5);
            }

            // 메뉴 항목에 파일 리스트 출력
            let selectoption = '';

            for (let i = 0; i < response.data.result.genieMoviedata.filelist.length; i++) {
                if (fileHour == response.data.result.genieMoviedata.filelist[i].slice(27, -5)) {
                    selectoption += `<option value="${response.data.result.genieMoviedata.filelist[i].slice(27, -5)}" selected>${response.data.result.genieMoviedata.filelist[i].slice(27, -5)}:00</option>`
                } else {
                    selectoption += `<option value="${response.data.result.genieMoviedata.filelist[i].slice(27, -5)}">${response.data.result.genieMoviedata.filelist[i].slice(27, -5)}:00</option>`
                }
            }
