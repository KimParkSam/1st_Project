window.addEventListener('DOMContentLoaded', event => {
    ...
    window.dateHourChange = () => {
        ...
        const url = '/melonRealChart/'+viewTime;
        ...
        axios({
            ...
            const fileday = response.data.result.melondata.filelist[response.data.result.melondata.filelist.length-1].slice(15, -8);
            dayTag.textContent = fileday;

            let fileHour = response.data.result.melondata.fileHour;
            if (fileHour == undefined) {
                fileHour = response.data.result.melondata.filelist[response.data.result.melondata.filelist.length-1].slice(26, -5);
            }
            ...
            for (let i = 0; i < response.data.result.melondata.filelist.length; i++) {
                if (fileHour == response.data.result.melondata.filelist[i].slice(26, -5)) {
                    selectoption += `<option value="${response.data.result.melondata.filelist[i].slice(26, -5)}" selected>${response.data.result.melondata.filelist[i].slice(26, -5)}:00</option>`
                } else {
                    selectoption += `<option value="${response.data.result.melondata.filelist[i].slice(26, -5)}">${response.data.result.melondata.filelist[i].slice(26, -5)}:00</option>`
                }
            }
